...
"xstate.after(TIME_BETWEEN_REQUESTS)#(machine).busy": { type: "xstate.after(TIME_BETWEEN_REQUESTS)#(machine).busy" };
...
"sendParentQueryError": "error.platform.(machine).filtering:invocation[0]";
        ...
        eventsCausingDelays: {
          "TIME_BETWEEN_REQUESTS": "" | "done.invoke.(machine).filtering:invocation[0]" | "done.invoke.(machine).hydration:invocation[0]" | "error.platform.(machine).filtering:invocation[0]" | "error.platform.(machine).hydration:invocation[0]";
        };
        eventsCausingGuards: {
